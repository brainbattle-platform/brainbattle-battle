...
import {
    CreateQuestionDto,
    ImportLearningQuestionsDto,
    LearningQuestionItemDto,
    ListQuestionsDto,
    UpdateQuestionDto,
} from './dto';
...
export class QuestionService {
    ...
    async importFromLearning(userId: string, dto: ImportLearningQuestionsDto) {
        const externalSource = dto.externalSource ?? 'LEARNING';

        const results: Array<Awaited<ReturnType<typeof this.importOneLearningQuestion>>> =
            [];

        for (const item of dto.questions) {
            const result = await this.importOneLearningQuestion(
                userId,
                externalSource,
                item,
            );
        ...
        return {
            ...
            versioned: results.filter(
                (item) => item.action === 'CREATED_NEW_VERSION',
            ).length,
            skipped: results.filter(
                (item) => item.action === 'SKIPPED_DUPLICATE',
            ).length,
            invalid: results.filter((item) => item.validation?.valid === false).length,
            items: results,
        };
    }
    ...
    private async importOneLearningQuestion(
        userId: string,
        externalSource: string,
        item: LearningQuestionItemDto,
    ) {
        const normalized = this.normalizeLearningQuestion(externalSource, item);
        const contentHash =
            item.contentHash ?? this.computeContentHash(normalized);

        const duplicate = await this.prisma.battleQuestion.findFirst({
            where: {
                externalSource,
        ...
        if (duplicate) {
            return {
                ...
                question: toQuestionResponse(duplicate),
                validation: validateBattleQuestion(duplicate),
            };
        }

        const latestSameExternal = await this.prisma.battleQuestion.findFirst({
            where: {
                externalSource,
                externalQuestionId: item.externalQuestionId,
            },
        ...
        const questionGroupId = latestSameExternal?.questionGroupId ?? randomUUID();
        const version = latestSameExternal ? latestSameExternal.version + 1 : 1;

        const created = await this.prisma.$transaction(async (tx) => {
            const question = await tx.battleQuestion.create({
                data: {
                    questionGroupId,
                    version,

                    source: QuestionSource.IMPORTED_FROM_LEARNING,
                    externalSource,
                    externalQuestionId: item.externalQuestionId,
            ...
            await this.replaceQuestionChildren(tx, question.id, normalized);

            await tx.questionReviewLog.create({
                data: {
                    questionId: question.id,
                    action: latestSameExternal
                        ? `IMPORTED_FROM_LEARNING_NEW_VERSION:${latestSameExternal.id}`
                        : 'IMPORTED_FROM_LEARNING',
                    actorUserId: userId,
                    note: `externalSource=${externalSource}; externalQuestionId=${item.externalQuestionId}`,
                },
            });
        ...
        const validation = validateBattleQuestion(created);
        ...
        if (validation.valid) {
            const submitted = await this.prisma.$transaction(async (tx) => {
                ...
                await tx.questionReviewLog.create({
                    data: {
                        questionId: created.id,
                        action: 'AUTO_SUBMITTED_FOR_REVIEW_AFTER_IMPORT',
                        actorUserId: userId,
                    },
                });
            ...
            return {
                action: latestSameExternal ? 'CREATED_NEW_VERSION' : 'CREATED',
                externalQuestionId: item.externalQuestionId,
                questionId: submitted.id,
                question: toQuestionResponse(submitted),
                validation: validateBattleQuestion(submitted),
            };
        }
    ...
    private normalizeLearningQuestion(
        externalSource: string,
        item: LearningQuestionItemDto,
    ) {
        return {
            ...
            correctOptionKey:
                item.correctAnswer.optionKey?.trim().toUpperCase() || null,
            acceptedAnswers: this.normalizeAnswers(
                item.correctAnswer.acceptedAnswers,
            ),

            maxTimeSec: item.estimatedTimeSec ?? this.resolveDefaultTimeSec(item),
            baseScore: 100,
            speedBonus: 50,
    ...
    private resolveDefaultTimeSec(item: LearningQuestionItemDto) {
        const maxMediaDuration = Math.max(
            0,
            ...(item.media ?? []).map((media) => media.durationSec ?? 0),
        );

        if (item.skill === 'LISTENING') {
            if (item.type === 'FILL_BLANK') {
                return Math.min(Math.max(maxMediaDuration + 10, 15), 30);
            }

            return Math.min(Math.max(maxMediaDuration + 6, 10), 25);
        }
