...
export class AdminQuestionController {
    ...
    @Post('import/learning')
    @ApiOperation({
        summary: 'Import questions from Learning service payload',
        description:
            'Imported questions are normalized into Battle Question Bank. Valid questions become PENDING_REVIEW, invalid questions remain DRAFT. Admin must approve before battle use.',
    })
    importFromLearning(
        @CurrentUser() user: AuthUser,
        @Body() dto: ImportLearningQuestionsDto,
    ) {
        this.assertAdmin(user);
        return this.questionService.importFromLearning(user.id, dto);
    }
    @Post()
